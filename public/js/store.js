...
export function setPhotos(p)    { state.photos = p; notify(); }
export function setAlbums(a)    { state.albums = a; notify(); }
export function setPagination(pg) { state.pagination = pg; notify(); }
export function setCurrentAlbum(a)  { state.currentAlbum = a; state.currentPage = 1; notify(); }
export function setCurrentSort(s)   { state.currentSort = s; state.currentPage = 1; notify(); }
export function setCurrentSearch(q) { state.currentSearch = q; state.currentPage = 1; notify(); }
export function setCurrentTimeFilter(t) { state.currentTimeFilter = t; state.currentPage = 1; notify(); }
export function setCurrentMediaType(m)  { state.currentMediaType = m; state.currentPage = 1; notify(); }
